...
class ARWorld extends THREE.Object3D {
  constructor(opts) {
    super();
  ...
  update() {
    ...
    // your shader_uniforms update here
    this.prevTime = time;
  }

  getFeatureDesc(feat) {
    // same code you had for this method
  }

  clearSelection() {
    if (this.selection) {
      this.selection.material.emissive.setHex(this.selection.currentHex);
      this.parent.remove(this.selection.sdf); // remove from parent (scene)
      this.selection = null;
    }
  }

  updateSelection(object) {
    // your existing logic here, but use `this` and `this.opts.camera`
    // and traverse `this.parent` or pass scene explicitly if needed
  }

  update_scene_label() {
    // your existing logic for labels, adapted to instance
  }
}
